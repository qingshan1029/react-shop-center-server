...
cloudinary.config({
  ...
  cloud_name: process.env.REACT_IMAGE_UPLOAD_CLOUD_NAME,
  api_key: process.env.REACT_IMAGE_UPLOAD_API_KEY,
  api_secret: process.env.REACT_IMAGE_UPLOAD_API_SECRET
})

console.log(process.env.REACT_IMAGE_UPLOAD_CLOUD_NAME)
console.log(process.env.REACT_IMAGE_UPLOAD_API_KEY)
console.log(process.env.REACT_IMAGE_UPLOAD_API_SECRET)
//GET /products/:id
router.delete('/products/delete/:id', function (req, res, next) {
  let productId2 = req.params.id
  const {productId} = req.body
  console.log('delete id  =  ' + productId)
  console.log('delete id 2  =  ' + productId2)

  Product.deleteProductByID(productId, function (e, item){
    if (e) {
...
//POST /products/image-upload /products/image-upload
router.post('/products/image-upload', function (req, res, next) {
...
router.post('/products/add', function (req, res, next) {
  const { imagePath, title, description, department, category, price, color, size, quantity } = req.body
  ...
  if (missingFieldErrors) {
    let err = new TypedError('add product error', 400, 'missing_field', {
  ...
  Product.filterProductByTitle(title, function (error, product) {
    if (error) return next(error)
    if (product.count > 0) {
      console.log(product)

      let err = new TypedError('add product error', 409, 'invalid_field', {
        message: error
      })
      return next(err)
    }
